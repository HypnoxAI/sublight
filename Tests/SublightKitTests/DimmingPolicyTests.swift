...
final class DimmingPolicyTests: XCTestCase {
    ...
    func testEffectiveRunningMatrix() {
        XCTAssertFalse(
            DimmingPolicy.effectiveRunning(userEnabled: false, systemSuspended: false))
        XCTAssertFalse(
            DimmingPolicy.effectiveRunning(userEnabled: false, systemSuspended: true))
        XCTAssertTrue(
            DimmingPolicy.effectiveRunning(userEnabled: true, systemSuspended: false))
        XCTAssertFalse(
            DimmingPolicy.effectiveRunning(userEnabled: true, systemSuspended: true))
    }
    ...
    func testScheduleTransitionMatrix() {
        XCTAssertEqual(
            DimmingPolicy.scheduleTransition(enteringWindow: true, consentGranted: true),
            .engage)
        XCTAssertEqual(
            DimmingPolicy.scheduleTransition(enteringWindow: true, consentGranted: false),
            .deferForConsent)
        XCTAssertEqual(
            DimmingPolicy.scheduleTransition(enteringWindow: false, consentGranted: true),
            .disengage)
        XCTAssertEqual(
            DimmingPolicy.scheduleTransition(
                enteringWindow: false, consentGranted: false), .disengage,
    ...
    func testDeferredConsentLifecycleAcrossAScheduleTransition() throws {
        ...
        let entering = DimmingPolicy.scheduleTransition(
            enteringWindow: true,
        ...
        for hz in [3.0, 6.0, 8.0] {
            XCTAssertEqual(
                DimmingPolicy.glyphFraction(
                    userEnabled: false, systemSuspended: false, frequencyHz: hz), 0)
            XCTAssertEqual(
                DimmingPolicy.glyphFraction(
                    userEnabled: false, systemSuspended: true, frequencyHz: hz), 0)
            XCTAssertEqual(
                DimmingPolicy.glyphFraction(
                    userEnabled: true, systemSuspended: true, frequencyHz: hz), 0,
    ...
    func testPresetsAreThreeSixAndTheCeiling() {
        ...
        XCTAssertEqual(
            FrequencyPreset.high, DitherEngine.maxStableFrequencyHz,
