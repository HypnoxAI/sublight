...
final class APISurfaceTests: XCTestCase {
    ...
    private var good: [String: String?] {
        Dictionary(
            uniqueKeysWithValues: APISurface.expectedEncodings.map {
                ($0.selector, Optional($0.encoding))
            })
    }

    func testAllMatchingEncodingsPass() {
        let r = APISurface.evaluate(
            frameworkLoaded: true, classPresent: true, encodings: good, macOSBuild: build)
    ...
    func testFadeSpeedEncodedAsDoubleFails() {
        ...
        let r = APISurface.evaluate(
            frameworkLoaded: true, classPresent: true, encodings: enc, macOSBuild: build)
        XCTAssertFalse(r.passed)
        XCTAssertEqual(r.failures.count, 1)
        XCTAssertEqual(
            r.failures[0].selector, "setBrightness:fadeSpeed:commit:forKeyboard:")
        XCTAssertEqual(
            r.failures[0].kind,
            .encodingMismatch(
                expected: "B36@0:8f16i20B24Q28", actual: "B40@0:8f16d20B28Q32"))
    ...
    func testMissingSelectorFails() {
        ...
        XCTAssertEqual(
            r.failures,
            [.init(selector: "backlightLevelForKeyboard:", kind: .selectorMissing)])
    }
    ...
    func testEveryMismatchIsReportedIndividually() {
        ...
        XCTAssertEqual(
            Set(r.failures.map(\.selector)),
            ["brightnessForKeyboard:", "enableAutoBrightness:forKeyboard:"])
    }

    func testClassMissingFailsWithoutJudgingSelectors() {
        let r = APISurface.evaluate(
            frameworkLoaded: true, classPresent: false, encodings: [:], macOSBuild: build)
        XCTAssertFalse(r.passed)
    ...
    func testFrameworkNotLoadableFails() {
        let r = APISurface.evaluate(
            frameworkLoaded: false, classPresent: false, encodings: [:], macOSBuild: build
        )
        XCTAssertEqual(r.failures.map(\.kind), [.frameworkNotLoadable])
    }

    func testExpectedTableMatchesTheArm64Finding() {
        let fade = APISurface.expectedEncodings.first {
            $0.selector == "setBrightness:fadeSpeed:commit:forKeyboard:"
        }
        XCTAssertEqual(
            fade?.encoding, "B36@0:8f16i20B24Q28",
            "fadeSpeed is a 32-bit int (i), not a float")
    }
}
