...
| 8d | **State-reflecting menu-bar icon** | Glyph at full strength while dimming, faded back when idle. Opacity, not colour — stays a template image. | **done** |
...
| 8a | **App icon & identity** | Ascending golden-ratio wave through a keyboard, inverting the keys it crosses (the dither engine, drawn). Masters in `assets/icons/`; mono template for the menu bar, amber-traced version for the Dock. | **done** |
