...
The status item is drawn in code (`StatusGlyph`, in the kit) rather than bundled
as an asset, and its fill level is the frequency in use bucketed to the nearest
preset (`DimmingPolicy.glyphFraction`, boundaries at the preset midpoints 4.5
and 7.0 Hz):

![Sublight menu bar icon states](../assets/icons/sublight-menubar-states.png)

Hollow means not dimming — off, or suspended by the system (§6.2). Keys fill
from the right: 0 / 3 / 5 / 8 of 10 for Off / Low / Medium / High. It is a
template image, so macOS handles light, dark, and reduced-transparency tinting.
The legend is regenerated from the drawing code itself via
`sublight-cli glyph render`, so this document and the icon cannot disagree.
